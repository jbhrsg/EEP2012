    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {                
                ...
                string DormID = Request.QueryString["DormID"];
                string CustID = Request.QueryString["CustID"];
                string TranType = Request.QueryString["TranType"];
                string InOutStatus = Request.QueryString["InOutStatus"];
                string RoomID = Request.QueryString["RoomID"];

                string ReportPath = "JB_JCS/REPORT/JCS/RoomerData.rdlc";//報表路徑

                var parameters = new List<object>();
                parameters.Add(DormID + "," + CustID + "," + TranType + "," + InOutStatus + "," + RoomID);
                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sJCS", "procReportRoomerList", parameters);
                ...
                try
                {
                    this.ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("DormID", DormID) });
