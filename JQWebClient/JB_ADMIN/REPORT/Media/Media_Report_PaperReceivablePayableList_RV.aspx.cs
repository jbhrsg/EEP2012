    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (this.ClientQueryString != "")
            {
                ...
                if (ReportType == "1")
                {
                ReportPath = "JB_ADMIN/REPORT/Media/Media_Report_PaperReceivablePayableList_Details.rdlc";
                }
                else if (ReportType == "2")
                {
                    ReportPath = "JB_ADMIN/REPORT/Media/Media_Report_PaperReceivablePayableList_Summary.rdlc";
                }
                else if (ReportType == "3")
                {
                    ReportPath = "JB_ADMIN/REPORT/Media/Media_Report_PaperReceivablePayableList_SalesID.rdlc";
                }
                else if (ReportType == "4")
                {
                    ReportPath = "JB_ADMIN/REPORT/Media/Media_Report_PaperReceivablePayableList_Month.rdlc";
                }
               

                var parameters = new List<object>();
                parameters.Add(CustNO + "," + SalesID + "," + SalesDateFrom + "," + SalesDateTo + "," + NewsTypeID + "," + NewsAreaID + "," + NewsPublishID + "," + ReportType);
                //回json string

                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sMedia_Report_PaperReceivablePayableList", "GetReportData", parameters);
