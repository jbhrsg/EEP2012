    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (this.ClientQueryString != "")
            {
                ...
                string ReportPath = "";
                ReportPath = "JB_ADMIN/REPORT/Media/Media_Report_Envelope.rdlc";//報表路徑

                var parameters = new List<object>();
                parameters.Add(sCustNO);

                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sMedia_Report_Envelope", "GetERPCustomers", parameters);
