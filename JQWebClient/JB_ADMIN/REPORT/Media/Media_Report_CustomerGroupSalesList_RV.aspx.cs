    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {             
                ...
                string DMTypeID = Request.QueryString["DMTypeID"];
                string ViewAreaID = Request.QueryString["ViewAreaID"];
                string ReportType = Request.QueryString["ReportType"];//報表格式	1明細 2彙總
               ...
               // string InsGroupName = Request.QueryString["InsGroupName"];
                ...
                if (ReportType == "1")
                {
                ReportPath = "JB_ADMIN/REPORT/Media/Media_Report_CustomerGroupSalesDetails.rdlc";//客戶別銷貨明細表
                }
                else if (ReportType == "2")
                {
                    ReportPath = "JB_ADMIN/REPORT/Media/Media_Report_CustomerGroupSalesSummary.rdlc";//客戶別銷貨彙總表
                }

                var parameters = new List<object>();
                parameters.Add(CustNO + "," + SalesID + "," + SalesDateFrom + "," + SalesDateTo + "," + SalesTypeID + "," + DMTypeID + "," + ViewAreaID + "," + ReportType + "," + NewsAreaID + "," + LetterType);
                //回json string

                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sMedia_Report_CustomerGroupSalesList", "GetReportData", parameters);
