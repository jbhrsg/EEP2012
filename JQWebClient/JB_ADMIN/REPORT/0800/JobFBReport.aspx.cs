    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {                
                ...
                string IsMark = Request.QueryString["IsMark"];
                string siAutoKey = Request.QueryString["siAutoKey"];
                string sDate = Request.QueryString["sDate"];

                ReportViewer1.LocalReport.DisplayName = sDate;

                string ReportPath = "JB_ADMIN/REPORT/0800/JobFB.rdlc";//報表路徑
                var parameters = new List<object>();
                parameters.Add(IsMark + "," + siAutoKey );

                this.ReportViewer1.LocalReport.ReportPath = ReportPath;
                this.ReportViewer1.LocalReport.DataSources.Clear();

                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sJOB0800", "PublishingJobFBExcel", parameters);
                ...
                this.ReportViewer1.LocalReport.DataSources.Add(new Microsoft.Reporting.WebForms.ReportDataSource("PublishingJobFB", dt));
