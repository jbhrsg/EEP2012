    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {                
                ...
                string YearMonthS = Request.QueryString["YearMonthS"];
                string YearMonthE = Request.QueryString["YearMonthE"];
                string CustomerShortName = Request.QueryString["CustomerShortName"];

                string ReportPath = "JB_ADMIN/REPORT/JBRecruit/EmpSalary.rdlc";//報表路徑

                var parameters = new List<object>();
                parameters.Add(CustomerID + "," + YearMonthS + "," + YearMonthE);
                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sJBRecruitEmpSalary2", "SelectEmpSalary2", parameters);
                ...
                try
                {
                    this.ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("CustomerShortName", CustomerShortName) });                                   
                }
                catch { }

                this.ReportViewer1.LocalReport.DataSources.Add(new Microsoft.Reporting.WebForms.ReportDataSource("NewDataSet", dt));
                this.ReportViewer1.LocalReport.Refresh();
                this.ReportViewer1.LocalReport.DisplayName = CustomerShortName;
            }
        }
    }
