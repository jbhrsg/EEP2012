    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {                
                ...
                string ExportDate = Request.QueryString["ExportDate"];

                string ReportPath = "JB_ADMIN/REPORT/OvertimeMeal/BankRemit1.rdlc";//報表路徑

                var parameters = new List<object>();
                parameters.Add(YearMonth + "," + ExportDate );
                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sHRMAttendOverTimeMealQuery", "ReportDelayLunch", parameters);
                ...
                try
                {
                    this.ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("ExportDate", ExportDate) });                                   
                }
                catch { }
