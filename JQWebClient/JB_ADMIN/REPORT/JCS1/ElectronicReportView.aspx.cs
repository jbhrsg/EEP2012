    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {                
                ...
                string YM = Request.QueryString["YM"];
                string CustID = Request.QueryString["CustID"];
                string InvoiceType = Request.QueryString["InvoiceType"];
                string IsElec = Request.QueryString["IsElec"];
                string ReportPath = "JB_ADMIN/REPORT/JCS1/ElectronicData.rdlc";//報表路徑

                var parameters = new List<object>();
                parameters.Add(YM + "," + CustID + "," + InvoiceType + "," + IsElec);
                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sJCS1", "procReportJCS1Electronic", parameters);
