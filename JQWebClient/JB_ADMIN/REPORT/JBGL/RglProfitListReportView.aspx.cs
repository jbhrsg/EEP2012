    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {                
                ...
                string CostCenterText = Request.QueryString["CostCenterText"];
                string JQDate1 = Request.QueryString["SDate"];
                string JQDate2 = Request.QueryString["EDate"];
                string CompanyText = Request.QueryString["CompanyText"];
                string IsDiff = Request.QueryString["IsDiff"];//----0 損益表 ,1 損益平均數差異分析 ,2 損益累積差異分析
                string IsEng = Request.QueryString["IsEng"];//----0正常表 ,1英文版

                string sDate = JQDate1 +"~"+ JQDate2;

                string ReportPath = "JB_ADMIN/REPORT/JBGL/ProfitList.rdlc";//報表路徑

                if (IsDiff == "1")
                {
                    ReportPath = "JB_ADMIN/REPORT/JBGL/ProfitListCompare.rdlc";
                }
                else if (IsDiff == "2")
                {
                    ReportPath = "JB_ADMIN/REPORT/JBGL/ProfitListCompare2.rdlc";
                }

                var parameters = new List<object>();
                parameters.Add(CompanyID + "," + VoucherID + "," + CostCenterID + "," + JQDate1 + "," + JQDate2 + "," + IsDiff + "," + IsEng);
                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sRVoucherList", "ReportProfitList", parameters);
                ...
                try
                {
                    ...
                    this.ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("CostCenterText", CostCenterText) });
