    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {                
                ...
                string SalesTeamText = Request.QueryString["SalesTeamText"];


                string ReportPath = "JB_ADMIN/REPORT/JBHunter/JobRequireReport.rdlc";//報表路徑
                var parameters = new List<object>();
                parameters.Add(CustID + "," + JobName + "," + HunterID + "," + SalesTeamID);

                this.ReportViewer1.LocalReport.DisplayName = SalesTeamText;

                this.ReportViewer1.LocalReport.ReportPath = ReportPath;
                this.ReportViewer1.LocalReport.DataSources.Clear();

                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sCustomersJobs", "procReportJobRequire", parameters);
                ...
                try
                {
                    this.ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("SalesTeamText", SalesTeamText) });

                }
                catch { }

                this.ReportViewer1.LocalReport.DataSources.Add(new Microsoft.Reporting.WebForms.ReportDataSource("JobRequire", dt));
