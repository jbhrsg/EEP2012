    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {                
                ...
                string ReportPath = "";
                if(ReportType=="1")
                ReportPath = "JB_ADMIN/REPORT/ERP/ERP_Report_CashTransferWarrantDetails_CustomerID.rdlc";//報表路徑
                else if (ReportType == "2")
                ReportPath = "JB_ADMIN/REPORT/ERP/ERP_Report_CashTransferWarrantDetails_CashNO.rdlc";//報表路徑

                var parameters = new List<object>();
                parameters.Add(CustomerID + "," + WarrantDateFrom + "," + WarrantDateTo + "," + ReportType + "," + InsGroupID);
                //回json string
                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sERP_Report_CashTransferWarrantDetails", "GetReportData", parameters);
