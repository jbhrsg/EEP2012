    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (this.ClientQueryString != "")
            {
                ...
                string ReportPath = "";
                ReportPath = "JB_ADMIN/REPORT/ERP/ERP_Report_PrintEnvelope.rdlc";//報表路徑

                var parameters = new List<object>();
                parameters.Add(CustomerID + "," + SalesTypeID);

                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sERP_Report_PrintEnvelope", "GetCustomerSaleType", parameters);
