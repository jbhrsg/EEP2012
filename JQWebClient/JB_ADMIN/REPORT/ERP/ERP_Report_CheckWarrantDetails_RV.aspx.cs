    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {                
                ...
                string ReportPath = "JB_ADMIN/REPORT/ERP/ERP_Report_CheckWarrantDetails.rdlc";

                var parameters = new List<object>();
                parameters.Add(CustomerID + "," + InsGroupID + "," + WarrantDateFrom + "," + WarrantDateTo + "," + AccountID);
                //回json string
                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sERP_Report_CheckWarrantDetails", "GetReportData", parameters);
                ...
                {
                    if (i != 0 && dt.Rows[i]["CheckNO"].ToString() == dt.Rows[i - 1]["CheckNO"].ToString() && dt.Rows[i]["WarrantNO"].ToString() == dt.Rows[i - 1]["WarrantNO"].ToString())
                        {
                            dt.Rows[i]["Amount"] = 0;
                        }
                }
