    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {                
                ...
                string iYear = Request.QueryString["iYear"];
                string SalesID = Request.QueryString["SalesID"];
                string WorkTime = Request.QueryString["WorkTime"];
                string NationalityID = Request.QueryString["NationalityID"];
                string EmployerName = Request.QueryString["EmployerName"];
                //string Month1 = Request.QueryString["Month1"];
                //string Month2 = Request.QueryString["Month2"];

                string ReportPath = "JB_ADMIN/REPORT/FWCRM/OrdersPeopleCount.rdlc";//報表路徑


                var parameters = new List<object>();
                parameters.Add(iYear + "," + SalesID + "," + WorkTime + "," + NationalityID + "," + EmployerName);

                var obj = client.CallMethod(EFClientTools.ClientUtility.ClientInfo, "sFWCRMOrdersPeopleCount", "ReportOrdersPeopleCount", parameters);
                ...
                try
                {
                    this.ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("iYear", iYear) });
                }
                catch { }
