...
//【關於時間的方法】
; (function ($) {
    $.jbIsDayStr = function (Str) {
        // 1~31
        var aNum = parseInt(Str, 10);
        if (isNaN(aNum)) return false;
        if (1 <= aNum && aNum <= 31) return true;
        return false;
    },
    ...
    $.jbIsTimeFormat_48 = function (Str) {
        /* 0000  -> O
         * 4759  -> O
         */
        var r = Str.match(/^(?:([0-3]\d|4[0-7])([0-5][0-9])|4800)$/);
        if (r == null) return false;
        else return true;
    },
...
function openQueryDeptcDialog() {
    var deptList = $("#DEPTC_CODE_Query").val();
    $.ajax({
        type: "POST",
        url: '../handler/jqDataHandle.ashx?RemoteName=_HRM_System_Share.COLDEF', //連接的Server端，command
...
function convertTreeData(data) {
    var json = JSON.parse(data);//將字串解析為json物件  
    var tree = {};//定義存放結果的tree物件  
    //將資料添加children屬性，並依據nodeId存放到tree物件中  
    $.each(json, function (i, obj) {
        this.children = [];//添加children屬性  
        tree[this.nodeId] = this;//nodeId為key，當前物件為value存放到tree物件中  
    });
    $.each(json, function (i, obj) {
        if (this.parentNodeId != 0) {
            tree[this.parentNodeId].children.push(this); //除了根節點之外，其餘節點都追加到父節點的chileren陣列中  
        }
    });
    var temp = [];
    temp.push(tree[1]);//獲取根節點，其餘節點不需要  
    tree = temp;
    /** 
     * 將給定的節點及其子節點(children)轉為easyui要求的結構 
     * @param {Object} tree1 
     * @return {TypeName}  
     */
    function toTree(tree1) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            var isClass = '';
            if (tree1[i].IS_CLASS != null) { isClass = tree1[i].IS_CLASS } else { isClass = '' };
            obj.attributes = {
                "nodeName": tree1[i].nodeName,
                "parentNodeId": tree1[i].parentNodeId,
                "IS_CLASS": isClass
            };
            obj.children = toTree(tree1[i].children);//遞迴children節點  
            arr.push(obj);
        }
        return arr;
    }
    tree = toTree(tree);
    result = JSON.stringify(tree);//得到的就是最終的easyui要求的格式  
...
//【關於jbExcelFileImport的方法】
; (function ($) {
    var ggFnName = 'jbExcelFileImport';

    //檔案上傳
    //上傳檔案。完成後回傳【檔案路徑】(寫回【檔案路徑】)

    //檔案讀取
    //根據【檔案路徑】讀取Excel 。回傳SheetIndex、SheetArray、TitleArray、FilePath(寫回【檔案路徑】)

    //檔案匯入
    //根據【檔案路徑】、SheetIndex、TitleArray，傳入ServerMethod
    ...
    var fileUpload = function (target, newID) {
        var theOptions = getOptions(target);            //取得Options
        $.ajaxFileUpload({
            url: '../handler/JbExcelHandler.ashx?mode=ExcelFileSave',
            secureuri: false,
            fileElementId: newID,  //ID
            dataType: 'json',
            success: function (jsonResult) {
                if (!jsonResult.IsOK) alert(jsonResult.ErrorMsg);
                else {
                    setFilePath(target, jsonResult.Result);         //設定FilePath
                    theOptions.OnFileUploadSuccess.call(target);
                }
            },
    ...
    //轉換Sheet
    var changeSheetByName = function (target, sheetName, filePath) {
        //取得FilePath
        var theFilePath = getFilePath(target);
        filePath = filePath || theFilePath;
        readExcelFile(target, filePath, -1, sheetName);
    }

    //轉換Sheet
    var changeSheetByIndex = function (target, sheetIndex, filePath) {
        //取得FilePath
        var theFilePath = getFilePath(target);
        filePath = filePath || theFilePath;
        readExcelFile(target, filePath, sheetIndex, '');
    }

    //讀取檔案
    var readExcelFile = function (target, filePath, sheetIndex, sheetName) {
        var theOptions = getOptions(target);//取得Options
        $.ajax({
            type: 'post',
            url: '../handler/JbExcelHandler.ashx?mode=ExcelFileGetSheetTitle',
            data: { sheetIndex: sheetIndex, sheetName: sheetName, filePath: filePath },
            async: false,
            success: function (jsonStr) {
                var jsonResult = $.parseJSON(jsonStr);
                if (!jsonResult.IsOK) alert(jsonResult.ErrorMsg);
                else {
                    var theResult = jsonResult.Result;
                    //設定FilePath
                    setFilePath(target, theResult.FilePath);
                    //觸發事件
                    theOptions.OnGetTitleSuccess.call(target, theResult.SheetArray, theResult.TitleArray);
                }
            },
            beforeSend: function () { $.messager.progress({ msg: '執行中...' }); },
            complete: function () { $.messager.progress('close'); },
    ...
    //匯入作業
    var importFile = function (target, parameter, filePath) {
        var theFilePath = getFilePath(target);  //取得FilePath        
        filePath = filePath || theFilePath;
        ...
        $.ajax({
            url: '../handler/JbExcelHandler.ashx?mode=ExcelFileSheetImport',
            data: {
                filePath: filePath,
                remoteName: ImportParameter.remoteName,
                method: ImportParameter.method,
                sheetIndex: ImportParameter.sheetIndex,
                titleObject: $.toJSONString(ImportParameter.titleObject),
                parameters: ImportParameter.parameters
            },
            type: 'POST',
            async: true,
            success: function (jsonStr) { theOptions.OnImportSuccess(jsonStr); },
    ...
    $.fn[ggFnName].methods = {
        ...
        getFilePath: function (target) {
            return getFilePath($(target[0]));
        },
        changeSheetByIndex: function (target, sheetIndex, filePath) {
            target.each(function () {
                changeSheetByIndex($(this), sheetIndex, filePath);
            });
            return target;
        },
        changeSheetByName: function (target, sheetName, filePath) {
            target.each(function () {
                changeSheetByName($(this), sheetName, filePath);
            });
            return target;
        },
        importFile: function (target, parameter, filePath) {
            target.each(function () {
                importFile($(this), parameter, filePath);
            });
            return target;
        }
    }
    ...
    $.fn[ggFnName].defaults = {
        ...
        OnGetTitleSuccess: function (SheetArray, TitleArray) {
            alert('Array Length:' + TitleArray.length);
        },
        OnImportSuccess: function (jsonStr) {
            alert(jsonStr);
        }
...
; (function ($) {
    ...
    var deleteDate = function (target, date) {
        ...
        if (index != -1) {
            allDate = $.grep(allDate, function (n, i) { return i != index });
    ...
    var addDate = function (target, date) {
        ...
        if (theOptions.onBeforeAdd != undefined) {
            ...
            if (ans != undefined && ans.toString() == 'false') ans = false;
        }
    ...
    var remove = function (target) {
        ...
        var SelectArray = aSelect.find('option:selected');
        ...
        if ($(SelectArray).length > 0) {
            ...
            //刪除
            $.each(SelectArray, function (i, o) {
                if (deleteDate(target, $(o).data('date'))) {
                    if (!EventTrigger) EventTrigger = true;
                }
            });
            ...
            if (EventTrigger) {
                ...
                //加入資料變動事件
                var theOptions = options(target);
                if (theOptions.onDataChange != undefined) theOptions.onDataChange.call(target);
            }
        }
    };
    ...
    var onSelect = function (target, date) {
        ...
        //新增或刪除
        if (isExists(target, date)) EventTrigger = deleteDate(target, date);
        else EventTrigger = addDate(target, date);
...
; (function ($) {
    ...
    var createButton = function (target, Index) {
        ...
        var aMenu = $('<div>', { id: theOptions.id + theMenuMenuID + Index, width: theOptions.width, style: 'display:none;' });
