...
function getRemoteUrl(remoteName, tableName, includeRows, rows) {
    ...
    if (includeRows != undefined && includeRows == true) {
        if (rows != undefined && rows != "")
            return "../handler/jqDataHandle.ashx?RemoteName=" + remoteName + "&TableName=" + tableName + "&IncludeRows=true&rows=" + rows + param;
        else
            return "../handler/jqDataHandle.ashx?RemoteName=" + remoteName + "&TableName=" + tableName + "&IncludeRows=true" + param;
    }
    else {
        if (rows != undefined && rows != "")
            return "../handler/jqDataHandle.ashx?RemoteName=" + remoteName + "&TableName=" + tableName + "&rows=" + rows + param;
        else
            return "../handler/jqDataHandle.ashx?RemoteName=" + remoteName + "&TableName=" + tableName + param;
    }
};
...
function getDisplayRow(val, options, whereItem) {
    ...
    queryWord.whereString = valueField + " = " + formatQueryValue(val.toString(), "string", isNvarChar) + "";
...
function getDisplayValues(val, options, whereItem) {
    ...
    queryWord.whereString = valueField + " in (" + values.join(',') + ")";
    if (whereItem != undefined && whereItem != '') {
        queryWord.whereString += " and " + whereItem;
    }
    if (valueField != textField && val != undefined) {
...
(function ($) {
    $.fn.extend({
        resizeImg: function () {//由于使用了parent().parent()这样很特例的方式，这个方法只适用于在datagrid中使用image的format的图片，调用方法为datagrid的onloadsuccess事件中调用$(".info-image").resizeImg();
            this.each(function () {
                $(this).load(function () {
                    $(this).parent().parent().css({ "width": $(this).parent().parent().width() + 1, "height": $(this).parent().parent().height() + 1 });
                    $(this).parent().css({ "padding": "0px", "width": $(this).parent().parent().width(), "height": $(this).parent().parent().height() });
...
function infoimageonmouseover() {
    setTimeout(function () {
        var date = new Date();
        if ($(".info-imagecontainer:first").attr("canclose") != "false" || date - imagesettime > 1500) {
            $(".info-imagecontainer:first").fadeOut(1200);
        }
    }, 500);
};
var imagesettime = 0;
function infogridimageformatterset(target, val) {
    if (val != "") {
        if ($(".info-imagecontainer:first") != undefined && $(".info-imagecontainer:first").length > 0) {
            ...
            if (canshow) {
                ...
                $(".info-imagecontainer:first").attr("canclose", false);
            }
            $(".info-imagecontainer:first").attr("canshow", true);
        }
    }
};
//----------------------------------------------------------------------------------------------------------------
...
var commandview = $.extend({}, $.fn.datagrid.defaults.view, {
    addCommandColumn: function (target, index) {
        ...
        function _add(rowIndex) {
            ...
            tr.find('span.icon-view').unbind('.datagrid').bind('click.datagrid', function (e) {
            ...
            tr.find('span.icon-edit').unbind('.datagrid').bind('click.datagrid', function (e) {
...
$.extend($.fn.datagrid.defaults.editors, {
    ...
    inforefval: {
        ...
        resize: function (target, width) {
            var width = width - 30;
            $.data(target[0], "inforefval").refval.find("input.refval-text").width(width);
        }
    },
    yearMonth: {
        init: function (container, options) {
    ...
    textarea: {
        init: function (container, options) {
            ...
            var input = $('<textarea ' + maxLength + disabled + height + ' >').appendTo(container);
            return input;
        },
        getValue: function (target) {
            return $(target).val();
...
$.extend($.fn.datagrid.methods, {
                        ...
                        if (controlClass != undefined && controlClass.indexOf('info-options') == 0 && getInfolightOption($(this)).multiSelect) {
                            ...
                            for (i = 0; i < values.length; i++) {
                                ...
                                switch (condition) {
                                    ...
                                    case '>':
                                    case '<':
                                        optionsstring += fieldName + condition + formatQueryValue(values[i], dataType, isNvarChar); break;
                                    case '%': optionsstring += fieldName + " like " + nvarchar + "'" + values[i].toString().replace(/\'/g, "''") + "%'"; break;
                                    case '%%': optionsstring += fieldName + " like " + nvarchar + "'%" + values[i].toString().replace(/\'/g, "''") + "%'"; break;
                                    case 'in':
                            ...
                            switch (condition) {
                                ...
                                case '>':
                                case '<':
                                    where += fieldName + condition + formatQueryValue(value, dataType, isNvarChar); break;
                                default:
                            }
    ...
    setWhere: function (jq, where) {
        jq.each(function () {
            ...
            if ($(this).datagrid('options').url == '') {
                ...
                var url = getRemoteUrl(remoteName, tableName, true);
                $(this).datagrid({ loadFilter: defaultFilter, url: url });
            }
            else {
                $(this).datagrid('load');
            }
    ...
    getChangedDatas: function (jq, includeDetail) {
        ...
        if (changes.length != 0) {
            ...
            changedRows.tableName = getInfolightOption(grid).tableName;
        ...
        if (jq.length > 0) {
            ...
            if (recordLock) {
                ...
                $.ajax({
                    type: "POST",
                    dataType: 'json',
                    url: getRemoteUrl(remoteName, tableName),
                    data: "keys=" + $.toJSONString(keys) + "&rows=" + $.toJSONString(rows) + "&mode=recordlock&locktype=" + type + "&lockmode=" + mode,
                    cache: false,
                    async: false,
                    success: function (data) {
                        if (data.result == "idle") {
            ...
            if (recordLock) {
                ...
                if (rows && rows.length > 0) {
                    ...
                    $.ajax({
                        ...
                        dataType: 'text',
                        url: getRemoteUrl(remoteName, tableName),
                        data: "keys=" + $.toJSONString(keys) + "&rows=" + $.toJSONString(lockRows) + "&mode=recordlock&locktype=idle",
                        cache: false,
                        async: false,
                        success: function (data) {
    ...
    pasteDatasFromExcel: function (jq) {
        ...
        var label1 = $('<label>' + cname[0] + '</label >').appendTo(div1);
        var columnIndext = $("<input type=\"text\" value ='1'></input>").appendTo(div1).css('width', '30px');
        $('<br/>').appendTo(div1);
        $('<br/>').appendTo(div1);
        var label1 = $('<label>' + cname[1] + '</label >').appendTo(div1);
        var insertornott = $("<input checked=\"checked\" type=\"checkbox\"></input>").appendTo(div1);
        $('<br/>').appendTo(div1);
        $('<br/>').appendTo(div);
                                ...
                                if (selectedRow != undefined) {
                                    ...
                                    if (selectedIndex + i >= rowCount) {
                                        ...
                                        for (var field in newdata) {
                                            var editor = grid.datagrid('getEditor', { index: selectedIndex + i, field: field });
                                            if (editor) {
...
$.extend($.fn.tree.methods, {
    setWhere: function (jq, where) {
        jq.each(function () {
            ...
            var remoteName = getInfolightOption($(this)).remoteName;
            var tableName = getInfolightOption($(this)).tableName;
            var url = getRemoteUrl(remoteName, tableName, false, -1) + "&whereString=" + encodeURI(whereString);
            $(this).tree({ url: url });
            $(this).tree('reload');
        });
    }
...
function changeCondition(sender) {
    ...
    var currentCondition = $("#" + conditionId).text();
    var x;
    var nextCondition;
    for (x in queryConditions) {
    ...
    $("#" + conditionId)[0].innerHTML = nextCondition;
}
...
function initInfoDataGrid(datagrid, parentTableName, parentRow) {
    ...
    var totalWord = new Object();
    var totalParamters = new Object();
    var hasTotal = false;
    ...
    if (parentTableName != undefined) {
        var queryWord = new Object();
        queryWord.parentTableName = parentTableName;
    ...
    queryParams.queryWord = whereString;
    if (hasTotal) {
        showFooter = true;
    ...
    if (columnsHibeable != undefined && columnsHibeable == true) {
        ...
        var hideColumnMenu;
        if ($('#' + hideColumnMenuId) != undefined && $('#' + hideColumnMenuId).length > 0) {
            hideColumnMenu = $('#' + hideColumnMenuId);
        }
        else {
                    ...
                    if (keyValues != undefined && keyValues != null && keyValues != "") {
                        if (editMode != undefined && (editMode.toLowerCase() == "switch" || editMode.toLowerCase() == 'dialog')) {
                            if (getInfolightOption($(this)).parent == undefined || getInfolightOption($(this)).parent == "") {
                                var flowKey = Request.getQueryStringByName("FORM_PRESENTATION");
                                if (Request.getFlowStringByName(flowKey, "NAVIGATOR_MODE") == "2" || Request.getFlowStringByName(flowKey, "NAVMODE") == "2") {
...
function initInfoRefVal(info_refval) {
    ...
    if (fixTextbox == "true") {
        $(infoRefvaltext).keyup(function (sender) {
            if (form != undefined) {
    ...
    $(infoRefvalquerybutton).unbind().bind('click', function (event) {
        ...
        if (refval.length > 0) {
            ...
            if (whereItem != undefined && whereItem != '') {
                var queryWord = new Object();
                queryWord.whereString = whereItem;
                queryParams.queryWord = $.toJSONString(queryWord);
            ...
            if (dialogCenter != true) {
                ...
                //jbjob edit by serlina
                $(panel).dialog({ top: top, left: $(this).offset().left });
            }
            $(panel).dialog('open');
        }
            ...
            success: function (data) {
                if (data['result'] == "success") {
                    ...
                    if (onSuccess != undefined) {
                        ...
                        //jbjob edit by GG
                        onSuccess.call(infofileUpload, data["message"]);
                    }
                }
                    ...
                    if (onError != undefined) {
                        ...
                        //jbjob edit by GG
                        onError.call(infofileUpload, data["message"]);
                    }
                    else {
                        alert(data["message"]);
                    }
...
function infoTreeViewDataLoop(data, parentValue, sloopData, id, pid, text) {
    ...
    for (; i < lng; i++) {
        ...
        if (pvalue == parentValue) {
            ...
            loopData.id = data[i][id];
            loopData.pid = data[i][pid];
            loopData.text = data[i][text];
            sloopData[children].push(loopData);
...
function initInfoTreeView(treeview) {
    ...
    var url = getRemoteUrl(remoteName, tableName, false, -1);
    ...
    $(treeview).tree({
        ...
        loadFilter: function (data) {
            var r = [], hash = {}, d = {}, id = idField, pid = parentField, text = textField, children = "children", i = 0, j = 0, lng = data.length;
            if ($(this).attr('editMode') == undefined) {
                for (; i < lng; i++) {
                    var pvalue = data[i][pid];
                    if (pvalue == rootValue || ((pvalue == null || pvalue == "") && (rootValue == null || rootValue == ""))) {
                        var topData = new Object();
                        topData.id = data[i][id];
                        topData.pid = data[i][pid];
                        topData.text = data[i][text];
...
function initInfoOptions(target) {
    ...
    if (openDialog) {
        ...
        $(target).data("infooptions", { text: optionstext, button: optionsbutton, panel: optionspanel.find('.panel-body') });
...
function initInfoFLComment(target) {
    ...
    target.datagrid({
        ...
        columns: [[
                    ...
                    {
                        field: 'REMARK', title: $.fn.flcomment.defaults.remarkText, sortable: true, width: 120, formatter: function (value, row, index) {
...
function initInfoYearMonth(target) {
    ...
    if (type == 'year') {
        ...
        //最小年份的起始月份
        monthMinus = monthcur + 12 * overrideMinus - durationMinus;
        //最大年份的最终月份
        monthPlus = monthcur + monthwhile;
        ...
        if (overrideMinus == 0 && overridePlus == 0) {
            ...
            for (var i = monthMinus ; i <= monthcur + durationPlus; i++) {
                var d = new Object();
                d["label"] = format.replace(/yyyy/g, yearcur).replace(/yy/g, yy).replace(/mm/g, i).replace(/YYY/g, YYY).replace(/YY/g, YY);
                d["value"] = yearcur.toString() + i.toString();
                data.push(d);
            }
        }
        else {
            ...
            if (overrideMinus > 0) {
                ...
                for (var i = monthMinus ; i < 13; i++) {
                    var d = new Object();
                    d["label"] = format.replace(/yyyy/g, yearMinus).replace(/yy/g, yy).replace(/mm/g, i).replace(/YYY/g, YYY).replace(/YY/g, YY);
                    d["value"] = yearMinus.toString() + i.toString();
                    data.push(d);
                }
                ...
                for (var i = 1; i < overrideMinus; i++) {
                    for (var j = 1; j < 13; j++) {
                        var d = new Object();
                        d["label"] = format.replace(/yyyy/g, yearMinus + i).replace(/yy/g, yy).replace(/mm/g, j).replace(/YYY/g, YYY).replace(/YY/g, YY);
                        d["value"] = yearMinus.toString() + j.toString();
                        data.push(d);
                    }
                }
            }
            ...
            if (overridePlus > 0) {
                ...
                for (var i = 0; i < overridePlus; i++) {
                    for (var j = 1; j < 13; j++) {
                        var d = new Object();
                        d["label"] = format.replace(/yyyy/g, yearcur + i).replace(/yy/g, yy).replace(/mm/g, j).replace(/YYY/g, YYY).replace(/YY/g, YY);
                        d["value"] = yearcur.toString() + j.toString();
                        data.push(d);
                    }
                }
            }
            ...
            for (var i = 1 ; i <= monthPlus; i++) {
                var d = new Object();
                d["label"] = format.replace(/yyyy/g, yearPlus).replace(/yy/g, yy).replace(/mm/g, i).replace(/YYY/g, YYY).replace(/YY/g, YY);
                d["value"] = yearPlus.toString() + i.toString();
                data.push(d);
            }
...
function initinfoschedule(target) {
    ...
    $(target).schedule({
        ...
        onBeforeLoad: function () {
            ...
            $(this).schedule('options').queryWord = new Object();
...
$.fn.BatchMove = function (methodName, value) {
    if (typeof methodName == "string") {
        ...
        this.each(function () {
            ...
            $(this).BatchMove('initialize', options);
...
$.fn.BatchMove.methods = {
    ...
    batchMove: function (target) {
        ...
        if ($('#' + srcDataGrid) && dg && $('#' + srcDataGrid).length > 0 && dg.length > 0) {
            var srcrows = $('#' + srcDataGrid).datagrid('getChecked');
            if (srcSelectAll) {
                srcrows = $('#' + srcDataGrid).datagrid('getRows');
            }
    ...
    batchMoveOne: function (target) {
        ...
        var onEachMove = options.onEachMove;
        var dg = $('#' + desDataGrid);
        if ($('#' + srcDataGrid).length > 0 && $('#' + desDataGrid).length > 0) {
            var srcrow = $('#' + srcDataGrid).datagrid('getSelected');
            var newRow = new Object();
            for (var i = 0; i < matchColumns.length; i++) {
                var columns = matchColumns[i];
                var desColumn = columns.desColumn;
    ...
    Move: function (target) {
        ...
        var dg = $('#' + desDataGrid);
        if ($('#' + srcDataGrid) && dg && $('#' + srcDataGrid).length > 0 && dg.length > 0) {
            if (getInfolightOption($('#' + srcDataGrid)).multiSelect) {
    ...
    getValue: function (target) {
        var options = $(this).BatchMove('options');
    }
};
...
function gridDetailFormatter(index, row) {
    var datagrid = $(this);
...
function formatValue(val, row) {
    ...
    if (format != undefined && val != undefined) {
        if (format.toLowerCase().indexOf('image') == 0) {
            ...
            onclickstring += "window.open('../handler/JqFileHandler.ashx?File=" + encodeURI(vpath) + "', 'download');";
            var aval = '<a href="#" onclick="' + onclickstring + '">' + val + '</a>';
            ...
            var ret = '<div class="info-qrcode" infolight-options="size:' + size + ',render:\'' + render + '\',text:\'' + val + '\'" />';
            return ret;
...
function endEdit(dg) {
    ...
    if (dg.datagrid('validateAll')) {
        ...
        var changedDatas = [];
        var changedRows = new Object();
        changedRows.tableName = getInfolightOption(dg).tableName;
        ...
        if (getInfolightOption(dg).duplicateCheck && insertdRows != null && insertdRows.length > 0) {
            ...
            if (parent != undefined && parent != "" && autoApply != undefined && !autoApply) {
                ...
                $.ajax({
                    type: "POST",
                    url: url,
                    data: "data=" + $.toJSONString(changedDatas) + "&mode=duplicate",
                    cache: false,
                    async: false,
...
function copyDetailItem(dgid) {
    ...
    if (formid != undefined && formid.attr('copyItem') != undefined) {
        ...
        if (parentTableName != undefined) {
            var queryWord = new Object();
            queryWord.parentTableName = parentTableName;
            queryWord.parentRow = $.parseJSON(formid.attr('copyItem'));
            $.ajax({
                type: "POST",
                url: getRemoteUrl(remoteName, tableName, true),
                data: { queryWord: $.toJSONString(queryWord) },
...
function updateItem(dgid) {
    ...
    if (onUpdate != undefined) {
        var rowData = $(dgid).datagrid('getSelected');
        var flag = onUpdate.call($(dgid), rowData);
        if (flag != undefined && flag.toString() == 'false') {
            return;
        }
    }
    ...
    $.ajaxFileUpload({
        ...
        success: function (data) {
            if (typeof data == "string") {
                ...
                $(dgid).data('pagination', pagination).data('imported', true).datagrid({ url: '', pagination: false }).datagrid('loadData', []);
                for (var i = 0; i < data.rows.length; i++) {
...
function exportXlsx(dgid) {
    ...
    $.ajax({
        type: "POST",
        url: getDataUrl(),
        data: getRemoteParam({ mode: 'exportXlsx', title: title, columns: $.toJSONString(columns), queryWord: queryWord }, remoteName, tableName, true),
        cache: false,
        async: false,
        success: function (data) {
...
function getRemoteParam(param, remoteName, tableName, includeRows, rows) {
    ...
    if (param.queryWord) {
        param.equeryWord = jsencrypt('infolight', param.queryWord);
        //delete param.queryWord;
    }
    return remoteParam;
}
...
function importXlsx(dgid) {
    ...
    var columnsCount = 0;
    var allColumns = $(dgid).datagrid('options').columns;
    if (allColumns[0].length > 0) {
        columnsCount = allColumns[0].length;
    }
    if (columnsCount == 0) {
        $.messager.alert('Error', dgid + "沒有設定欄位!", 'error');
        return false;
    }
    importWindow.window({
        title: 'Excel 匯入資料',
        ...
        maximizable: false,
        resizable: false,
        href: '../InnerPages/ImportXlsx.html',
        width: 400,
        height: 180,
        modal: true,
        onLoad: function () {
            $(".easyui-linkbutton", this).data("datagrid", dgid);
            $("#columnsCount", this).val(columnsCount);
        }
    });
    importWindow.window('open');
}
...
function uploadXlsx() {
    var dgid = $(".easyui-linkbutton", "#importWindow").data("datagrid");
    var row = $("#startRowNo").val();
    var cell = $("#columnsCount").val();
    var remoteName = getInfolightOption($(dgid)).remoteName;
    var tableName = getInfolightOption($(dgid)).tableName;
    var oldData = $(dgid).datagrid('getData');
    $.fn.Error.errorCode = 1060;

    $.ajaxFileUpload({
        url: getDataUrl(), //需要链接到服务器地址   
        secureuri: false,
        data: getRemoteParam({ mode: "importXlsx", beginrow: row, columnsCount: cell }, remoteName, tableName, true),
        fileElementId: "importFileUpload", //文件选择框的id属性   
        dataType: 'json', //服务器返回的格式，可以是json   
        success: function (data) {
            if (typeof data == "string") {
...
function exportReport(dgid, remoteName, tableName, reportFileName, whereString, options) {
    ...
    if (RemoteName != "" && TableName != "" && ReportFileName != "") {
        if (ReportFileName.toLowerCase().indexOf('.rdlc') > 0) { //jquery 或者 sd 使用reportName
            ...
            //jbjob edit by serlina
            var url = "../ReportViewerTemplate.aspx?RemoteName=" + RemoteName + "&TableName=" + TableName + "&ReportPath=" + ReportFileName + "&WhereString=" + encodeURIComponent(WhereString) + "&WhereTextString=" + WhereTextString + "&userName=" + userName;
            ...
            $.ajax({
                ...
                data: { mode: 'Run', id: ReportFileName, type: 'report', remoteName: RemoteName, whereString: WhereString, whereTextString: WhereTextString, DataSetName: DataSetName, HeaderDataSetName: HeaderDataSetName, HeaderTableName: HeaderTableName },
                async: true,
...
function cancel(dgid) {
    ...
    if (!autoApply) {
        ...
        if ($(dgid).data('imported')) {
            var remoteName = getInfolightOption($(dgid)).remoteName;
            var tableName = getInfolightOption($(dgid)).tableName;
            $(dgid).datagrid({ url: getRemoteUrl(remoteName, tableName, true), pagination: $(dgid).data('pagination') });
...
function applyUpdates(dg) {
    var parent = getInfolightOption(dg).parent;
    if (parent != undefined && parent != '') {
        if ($('#' + parent + dataGrid_class).length > 0) {
            applyUpdates($('#' + parent + dataGrid_class));
            return;
        }
    }
    ...
    if (changedDatas.length > 0) {
        ...
        if (tableName != changedDatas[0].tableName) {
            var changedRows = new Object();
            changedRows.tableName = tableName;
            changedRows.inserted = [];
            changedRows.updated = [];
            changedRows.deleted = [];
            var editIndex = getEditIndex(dg);
            if (editIndex != -1) {
                ...
                changedRows.updated.push(dg.datagrid('getSelected'));
            }
        ...
        $.ajax({
            type: "POST",
            url: url,
            data: "data=" + $.toJSONString(changedDatas) + "&mode=update",
            cache: false,
            async: true,
            beforeSend: function () {
            ...
            success: function (data) {
                ...
                if (dg.data('imported')) {
                    var remoteName = getInfolightOption(dg).remoteName;
                    var tableName = getInfolightOption(dg).tableName;
                    dg.datagrid({ url: getRemoteUrl(remoteName, tableName, true), pagination: dg.data('pagination') });
            ...
            $(".info-combogrid", jq[0]).each(function () {
                ...
                if (field != undefined) {
                    if (rowData[field] == undefined || rowData[field] == '') {
                        ...
                        var cgurl = getRemoteUrl(cgremoteName, cgtableName, false);
                        var cgvalueField = getInfolightOption(combogrid).valueField;
                        var cgqueryWord = new Object();
                        cgqueryWord.whereString = cgvalueField + "='" + rowData[field] + "'";
                        $.fn.Error.errorCode = 1900;
                        $.ajax({
                            type: "POST",
                            url: cgurl,
                            data: "queryWord=" + $.toJSONString(cgqueryWord),
                            cache: false,
                            async: false,
                            success: function (a, b, c) {
                                //alert(a);
...
function changeFormState(fid, mode, keys, editMode) {
    if (mode == 'viewed') {
        ...
        if (editMode != undefined && editMode.toLowerCase() == 'continue') {
            $('.infosysbutton-c', fid).each(function (index) {
                $(this).hide();
            });
        }
...
function loadFormRow(form, rowData, mode) {
    //jbjob edit by GG 修改此方法回傳數值
    var returnFormData = new Object();
    ...
    if (rowData) {
        ...
        $('input,select,textarea', form).each(function () {
            ...
            if (format != undefined) {
                if (rowData[field] == undefined) {
                    ...
                    var aval = $('<input type="image" onclick="return false;" ></input>').insertAfter(this);
                    ...
                    onclickstring += "window.open('../handler/JqFileHandler.ashx?File=" + vpath + "', 'download');";
                ...
                //}
                //formData[field] = rowData[field];
                // jbjob 2014/9/18 add by serlina
                //jbjob edit by serlina 當dateform class 為datebox 且未設定 formate 格式一律帶 yyyy/mm/dd
                if ($(this).hasClass('easyui-datebox')) {
                    if (format && format.indexOf('YY') == 0) $(this).datebox('rocYear', true);
                    else formData[field] = getFormatValue(rowData[field], 'yyyy/mm/dd');
                }
                else formData[field] = rowData[field];
            }
        });
        ...
        //jbjob edit by serlina 
        $(".info-jqte", form).each(function () {
            var textarea = $(this);
            var field = getInfolightOption(textarea).field;
            if (field != undefined) {
                textarea.context.value = $('<div/>').html(rowData[field]).text();  //解碼
                textarea.jqteVal($('<div/>').html(rowData[field]).text()); //放在元件內
            }
        });

        //jbjob edit by GG 為了formData
...
function openForm(fid, rowData, mode, editmode, keys) {
    ...
    if (tableName != undefined) {
        ...
        //jbjob edit by GG 將數值取回
        var formData = loadFormRow(form, rowData, mode);
        ...
        if (onLoadSuccess) {
            ...
            //jbjob edit by GG
            onLoadSuccess.call(form, formData);
        }
    }
};
...
function applyForm(fid) {
    ...
    if (form.form('validateForm')) {
        var changedDatas = [];
        var changedRows = new Object();
        changedRows.tableName = getInfolightOption(form).tableName;
        changedRows.inserted = [];
        changedRows.deleted = [];
        changedRows.updated = [];
        ...
        var tableName = getInfolightOption(form).tableName;
        var url = getRemoteUrl(remoteName, tableName, false);
        var duplicateCheckS = true;
                            ...
                            if (form.attr("switchGrid") != undefined && form.attr("switchGrid") != "") {
                                ...
                                if (flowFileName == "" && Request.getFlowStringByName(flowKey, "NAVIGATOR_MODE") != "2" && navMODE != "2") {
                                    if (mode == "inserted" && getInfolightOption(form).continueAdd) {
                                        ...
                                        form.form('clear');
                                        rowData = getDefaultValues(form);
                                        //rowData = getSeq(datagrid, form, rowData);
                                        form.form('load', rowData);
                                    }
                                    else {
                                        $(fid).window('close');
    ...
    $(":text,select,:checkbox", pnid).each(function () {
        ...
        if (defaultValue != undefined) {
            ...
            if (defaultValue.indexOf("client[") == 0) {
                ...
                defaultmethods[field + condition] = methodName;
                hasRemote = true;
            }
            else {
    ...
    $(eacht, element).each(function () {
        ...
        if (autoSeq != undefined) {
            ...
            if (autoSeq.length > 0) {
                ...
                var queryWord = $(grid).datagrid('options').queryParams.queryWord.replace(/&/g, '');
                var autoApply = getInfolightOption($(grid)).autoApply;
...
$.extend($.fn.validatebox.defaults.rules, {
    ...
    checkData: {
        validator: function (value, param) {
            ...
            queryWord.whereString = valueField + " = " + formatQueryValue(value.toString(), "string", isNvarChar);
            if (param.length >= 4) {
...
function getRootPath() {
    ...
    if ($('#_APPLICATIONPATH').length > 0) {
        return prePath + $('#_APPLICATIONPATH').val();
    }
    else {
...
function feedBack() {
    ...
    error.message = $(".errormessage").html().replace(/<br>/g, '\r\n').replace(/</g, "&lt;").replace(/>/g, "&gt;");
    ...
    $.ajax({
        ...
        data: { mode: 'feedback', error: $.toJSONString(error), stack: stack, source: (window.location.href != undefined && window.location.href != "") ? window.location.href.substr(window.location.href.lastIndexOf("/") + 1, window.location.href.length - window.location.href.lastIndexOf("/") - 2) : "" },
...
function userDefineLog(title, description, status) {
    ...
    $.ajax({
        type: "POST",
        url: "../handler/SystemHandle.ashx?Type=UserDefineLog&Title=" + title + "&Description=" + description + "&Status=" + status,
        cache: false,
        async: true
    });
}
...
function combogridQuery(param) {
    ...
    queryParams.queryWord = $.toJSONString(queryWord);
    datagrid.datagrid({ onLoadSuccess: function (data) { datagrid.datagrid('clearSelections'); } });
    datagrid.datagrid('load');
    //queryParams.queryWord = '';
};
                    ...
                    for (var i = 0; i < columnMatches.length; i++) {
                        if (rowdata) {
                            ...
                            if (value.indexOf("client[") == 0) {
                                ...
                                var methodName = value.replace("remote[", "").replace("]", "");
                                methods[columnMatches[i].field] = methodName;
                                hasRemote = true;
                            }
                            else {
        ...
        jq.each(function () {
            ...
            if (items != undefined && items.length > 0) {
                ...
                if (onWhere != undefined) {
                    var setWhere = onWhere.call(target);
                    queryWord.whereString = setWhere;
                }
                ...
                $.ajax({
                    type: "POST",
                    dataType: 'json',
                    url: getRemoteUrl(remoteName, tableName, true, -1),
                    data: "queryWord=" + $.toJSONString(queryWord),
                    cache: false,
                    async: false,
                    success: function (data) {
...
function insertTreeNodeOK(treeview) {
    ...
    var text = "";
    var row = new Object();
    var srcrow = new Object();
    $(":text", $(editDialog)).each(function () {
        if (getInfolightOption($(this)).field == idField) {
            id = $(this).val();
            row[idField] = encodeURIComponent(id);
            srcrow[idField] = id;
        }
        else if (getInfolightOption($(this)).field == textField) {
            text = $(this).val();
            row[textField] = encodeURIComponent(text);
            srcrow[textField] = text;
        }
        else {
            var value = $(this).val();
            var field = getInfolightOption($(this)).field;
            row[field] = encodeURIComponent(value);
            srcrow[field] = value;
        }
    });
    if (id != "") {
...
$.fn.drilldown.methods = {
    load: function (jq) {
        jq.each(function () {
            ...
            if ($('#' + drilldown)) {
                drilldown = $('#' + drilldown);
                var drillfieldsstring = getInfolightOption($(this)).drillfields;
...
function saveHtmlToDB(jqhtmlId, kvs) {
    ...
    var changedData = [];
    var changedRows = new Object();
    changedRows.tableName = infolightOptions.tableName;
    changedRows.deleted = [];
    changedRows.inserted = [];
    changedRows.updated = [];
...
$.fn.pivotTable = function (methodName, value) {
    if (typeof methodName == "string") {
        ...
        this.each(function () {
            var options = methodName;
            $(this).pivotTable('initialize', options);
    ...
    load: function (jq) {
        jq.each(function () {
            ...
            if (options.remoteName != undefined && options.remoteName != "" && options.tableName != undefined) {
                ...
                var data = $(this).pivotTable('loadData');
        ...
        jq.each(function () {
            ...
            if (options.remoteName != undefined && options.remoteName != "" && options.tableName != undefined) {
                ...
                for (var i = 0; i < options.showColumns.length; i++) {
                    var column = new Object();
                    column.field = options.showColumns[i].field;
                    column.title = options.showColumns[i].caption;
                    columns.push(column);
                }
                var url = getRemoteUrl(options.remoteName, options.tableName, false); //呼叫Server取得數據的url
                $.ajax({
                    ...
                    data: "mode=export&columns=" + $.toJSONString(columns) + "&queryWord=" + encodeURIComponent($.toJSONString(queryWord)),
                    cache: false,
                    async: false,
                    success: function (data) {
                        window.open('../handler/JqFileHandler.ashx?File=' + data, 'download');
