...
public partial class InnerPages_EEPSingleSignOn : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else if (Request.QueryString["Type"] != null && Request.QueryString["Type"] == "flowtodo")
        {
            ...
            var redirectUrl = string.Format("{0}/{1}.aspx?{2}", this.Request.QueryString["Package"], this.Request.QueryString["Form"], param);
        ...
        {
            var serviceUrl = ConfigurationManager.AppSettings["EEPWebClient"];
            var redirectUrl = string.Format("{0}/{1}.aspx?ItemParam={2}", this.Request.QueryString["Package"], this.Request.QueryString["Form"], this.Request.QueryString["ItemParam"]);
            Page.ClientScript.RegisterStartupScript(typeof(string), string.Empty
                , string.Format("singleSignOn('{0}','{1}','{2}','{3}','{4}','{5}');", serviceUrl, redirectUrl, clientInfo.UserID, clientInfo.Password, clientInfo.Database, clientInfo.Solution)
                , true);
        }
