function renderMain() {
    ...
    $('#tabsMain').tabs({
        onSelect: function (title, index) {
            if (index == 1) {
                $('#tabsWorkFlow').tabs('select', 0);
                if (window.top.FlowRefreshInbox)
                    window.top.FlowRefreshInbox.call();
                if (window.top.FlowRefreshOutbox)
...
function changePassword() {
    ...
    changePasswordWindow.window({
        ...
        maximizable: false,
        resizable: false,
        closable: false,    //20210714 edit by serlina for QUAKER PASSWORD Expiry
        href: 'InnerPages/ChangePassword.aspx',
        width: 360, //20210910 edit by serlina for PASSWORD Length over 10
        height: 350,
        modal: true,
        onLoad: function () {
