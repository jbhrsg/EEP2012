...
public class JqDataHandle : IHttpHandler, IRequiresSessionState
{
    ...
    private void CallMethod(HttpContext context)
    {
        ...
        var jdo = new JqDataObject(context.Request.QueryString[Data]);
    ...
    private void UpdateData(HttpContext context)
    {
        ...
        var jdo = new JqDataObject(context.Request.QueryString[Data]);
        var masterTableName = context.Request.QueryString["TableName"];
    ...
    public void RecordLock(HttpContext context)
    {
        ...
        var jdo = new JqDataObject(context.Request.QueryString[Data]);
        jdo.TableName = context.Request.QueryString["TableName"];
    ...
    private void SelectData(HttpContext context)
    {
        ...
        if (!string.IsNullOrEmpty(queryWord))
        {
            ...
            {
                var masterJdo = new JqDataObject(context.Request.QueryString[Data]);
        ...
        int rowCount = 0;
        var includeRows = context.Request.QueryString["IncludeRows"];
    ...
    public void ExportData(HttpContext context)
    {
        ...
        //2016/05/16 edit by serlina export to 97-2003 format(將原轉出excel xml 格式 改為 97-2003 格式)
        //JQUtility.ExportToExcel(exportTable, HttpContext.Current.Server.MapPath(path), gridTitle, exportColumns, totals);
        NPOIHelper.JBExportToExcel(exportTable, HttpContext.Current.Server.MapPath(path), gridTitle, exportColumns, totals);
