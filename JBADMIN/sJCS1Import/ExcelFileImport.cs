...
namespace TheExcelFileImport
{
    ...
    public class ExcelFileImport
    {
        ...
        protected string GetErrorColumnName(SystemImportField aField)
        {
            ...
            return (this.ImportFields.Contains(aField) ? aField : SystemImportField.CustNo).ToString();
        }
        ...
        protected void InputValidate2(DataTable CheckTable, FieldProcessExecutor FieldValidate)
        {
            try
            {
                ...
                DeleteRoomerImport(connection, transaction);

                //寫入新的判斷
                InsertRoomerImport(connection, transaction, CheckTable);               
            }
        ...
        protected bool InputValidate(DataTable CheckTable, FieldProcessExecutor FieldValidate)
        {
            ...
            foreach (DataRow aRow in CheckTable.Rows)
            {
                ...
                catch (Exception ex) { aRow.SetColumnError(SystemImportField.CustNo.ToString(), "欄位驗證時錯誤"); }
            }
        ...
        /// <summary>新增寫入RoomerImport</summary>
        protected void InsertRoomerImport(IDbConnection connection, IDbTransaction transaction, DataTable aTable)
        {
            try
            {
                //新增寫入
                using (SqlBulkCopy bulkCopy = new SqlBulkCopy((SqlConnection)connection, SqlBulkCopyOptions.KeepIdentity, (SqlTransaction)transaction))
                {
                    ...
                    aTable.Columns.Add(new DataColumn { ColumnName = "CustomerID", DefaultValue = this.CustomerID });
                    ...
                    bulkCopy.DestinationTableName = "JCS1.dbo.[RoomerImport]";
        ...
        protected void DeleteRoomerImport(IDbConnection connection, IDbTransaction transaction)
        {
            SqlCommand commandDelete = new SqlCommand("", (SqlConnection)connection, (SqlTransaction)transaction);
            commandDelete.CommandText = @"
Delete	T
From	JCS1.[dbo].[RoomerImport] as T";
        ...
        protected bool DataUpload()
        {
            ...
            string SQL = @"
exec JCS1.dbo.procUpdateRoomerImport @CustomerID,@CreateBy";

            try
            {
                var Parameter = new ArrayList();
                Parameter.Add(new SqlParameter("@CustomerID", this.CustomerID));
    ...
    {
        /// <summary>員工號碼</summary>        
        [EnumAttribute(FieldName = "員工號碼", IsRequired = true)]
        CustNo,
        /// <summary>班別</summary>
        [EnumAttribute(FieldName = "班別", IsRequired = true, IsBinding = true)]
        CustClass,
