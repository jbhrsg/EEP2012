...
namespace _HRM_Attend_Normal_Overtime
{
    public partial class Component : DataModule
    {
        ...
        public object[] getEffectDate(object[] objParam)
        {
            ...
            try
            {
                effectDate = Convert.ToDateTime(DateTime.Parse(overtimeDate).AddMonths(2).ToString("yyyy/MM/01")).AddDays(-1).ToString("yyyy/MM/dd");
                //transaction.Commit();
        ...
        private bool DataTableDataInsert(DataTable aTable)
        {
            ...
            try
            {
                ...
                foreach (DataRow aRow in aTable.Rows)
                {
                    configSql = "select HOLIDAY_ID from HRM_ATTEND_OVERTIME_CONFIG " + "\r\n";
                    ...
                    DataSet dsHRM_ATTEND_OVERTIME_CONFIG = this.ExecuteSql(configSql, connection, transaction);
                    holidayID = int.Parse(dsHRM_ATTEND_OVERTIME_CONFIG.Tables[0].Rows[0]["HOLIDAY_ID"].ToString());
