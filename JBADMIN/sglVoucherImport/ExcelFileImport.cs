...
namespace TheExcelFileImport
{
    ...
    public class ExcelFileImport
    {
        ...
        protected string GetErrorColumnName(SystemImportField aField)
        {
            ...
            return (this.ImportFields.Contains(aField) ? aField : SystemImportField.Acno).ToString();
        }
        ...
        public TheJsonResult GetTheFileImportResult()
        {
            ...
            //金額與傳票日期驗證
            var checkData =InputValidate2(ExcelDataTable, aFieldProcess);
            if (checkData.Item1!="0")
            {
                //var ErrorList = ExcelDataTable.AsEnumerable().Where(m => m.HasErrors).ToList();
                return new TheJsonResult { IsOK = false, ErrorMsg = "金額驗證錯誤", Result = NPOIHelper.SetErrorMemoToSheet(ExcelDataTable, new MemoryStream(this.FileStream.ToArray()), SheetIndex, Headler) };
            }
            if (checkData.Item2 != "0")
            {
                //var ErrorList = ExcelDataTable.AsEnumerable().Where(m => m.HasErrors).ToList();
                return new TheJsonResult { IsOK = false, ErrorMsg = "傳票日期驗證錯誤", Result = NPOIHelper.SetErrorMemoToSheet(ExcelDataTable, new MemoryStream(this.FileStream.ToArray()), SheetIndex, Headler) };
            }
            ////寫入傳票資料
        ...
        protected bool InputValidate(DataTable CheckTable, FieldProcessExecutor FieldValidate)
        {
            ...
            foreach (DataRow aRow in CheckTable.Rows)
            {
                ...
                catch (Exception ex) { aRow.SetColumnError(SystemImportField.Acno.ToString(), "欄位驗證時錯誤"); }
            }
        ...
        /// <summary>欄位驗證</summary>   
        protected Tuple<string,string> InputValidate2(DataTable CheckTable, FieldProcessExecutor FieldValidate)
        {
            try
            {
                //整體資料表金額驗證
                ...
                DeleteVoucherImport(connection, transaction);

                //寫入新的判斷
                var DataAmt = InsertVoucherImport(connection, transaction, CheckTable);
                return Tuple.Create(DataAmt.Item1, DataAmt.Item2);
            }
            catch (Exception) { throw new TheUserDefinedException("整體資料表金額驗證錯誤"); }
        ...
        {
            //傳票日期是否已鎖檔
            var ValidateRow0 = GetVoucherDatelock(aRow);
            if (ValidateRow0[0].ToString() == "1")
            {
                aRow.SetColumnError(SystemImportField.VoucherDate.ToString(), "此傳票日期已鎖檔");
                return false;
            }
            ...
            if (ValidateRow == null)
            {
                aRow.SetColumnError(SystemImportField.Acno.ToString(), "此科目-明細不存在");
                return false;
            }
            else if (ValidateRow != null) //找到資料了
            {
                ...
                {
                    aRow.SetColumnError(SystemImportField.Acno.ToString(), "此項需要成本中心");
                    return false;
                }
            }            
...
where CompanyID=@CompanyID and Acno=@Acno and SubAcno=Isnull(@SubAcno,'') 
";
            var Parameter = new ArrayList();
            Parameter.Add(new SqlParameter("@CompanyID", this.CompanyID));
            Parameter.Add(new SqlParameter("@Acno", aRow[SystemImportField.Acno.ToString()]));
            Parameter.Add(new SqlParameter("@SubAcno", aRow[SystemImportField.SubAcno.ToString()]));
        ...
        /// <summary>傳票日期是否已鎖檔</summary>
        protected DataRow GetVoucherDatelock(DataRow aRow)
        {
            string SQL = @"
select Isnull((select case IsActive when 0 then '0' else '1' end from glVoucherLockYM 
where CompanyID=@CompanyID and LockYM=left(CONVERT(nvarchar(10),Cast(@VoucherDate as datetime),112),6) ),'0')
";
            var Parameter = new ArrayList();
            Parameter.Add(new SqlParameter("@CompanyID", this.CompanyID));
            Parameter.Add(new SqlParameter("@VoucherDate", aRow[SystemImportField.VoucherDate.ToString()]));
        ...
        protected Tuple<string, string> InsertVoucherImport(IDbConnection connection, IDbTransaction transaction, DataTable aTable)
        {
            try
            {
                //新增寫入
                using (SqlBulkCopy bulkCopy = new SqlBulkCopy((SqlConnection)connection, SqlBulkCopyOptions.KeepIdentity, (SqlTransaction)transaction))
                {
                    ...
                    bulkCopy.ColumnMappings.Add("VoucherID", "VoucherID");
                    ...
                    aTable.Columns.Add(new DataColumn { ColumnName = "CompanyID", DefaultValue = this.CompanyID });
                    aTable.Columns.Add(new DataColumn { ColumnName = "VoucherID", DefaultValue = this.VoucherID });
                    aTable.Columns.Add(new DataColumn { ColumnName = "cDate", DefaultValue = DateTime.Now });
                    bulkCopy.DestinationTableName = "dbo.[glVoucherImport]";
            ...
            //匯入資料金額驗證
                ...
                string SQL = @"
exec procReturnglVoucherImportData @UserID
";
                var Parameter = new ArrayList();
                ...
                return Tuple.Create(DataSet.Tables[0].Rows[0]["iCount"].ToString(), DataSet.Tables[1].Rows[0]["iCount2"].ToString());                
        }

        /// <summary>刪除舊資料</summary>
        protected void DeleteVoucherImport(IDbConnection connection, IDbTransaction transaction)
        {
            SqlCommand commandDelete = new SqlCommand("", (SqlConnection)connection, (SqlTransaction)transaction);
            commandDelete.CommandText = @"
Delete	T
From	[dbo].[glVoucherImport] as T
Where	UserID = @UserID";
        ...
        protected bool DataUpload()
        {
            ...
            string SQL = @"
exec procInsertglVoucherImportData @CompanyID,@VoucherID,@UserID,@CreateBy";
          
            try
            {
                var Parameter = new ArrayList();
                Parameter.Add(new SqlParameter("@CompanyID", this.CompanyID));
                Parameter.Add(new SqlParameter("@VoucherID", this.VoucherID));
                Parameter.Add(new SqlParameter("@UserID", this.UserID));
    ...
    public enum SystemImportField
    {
        ...
        [EnumAttribute(FieldName = "傳票日期", IsRequired = true)]
        VoucherDate = 1,
        /// <summary>借貸</summary>
        [EnumAttribute(FieldName = "借貸", IsRequired = true ,IsBinding=true)]
        BorrowLendType,
        /// <summary>科目</summary>
        [EnumAttribute(FieldName = "科目", IsRequired = true, IsBinding = true)]
        Acno,
        /// <summary>明細</summary>
        [EnumAttribute(FieldName = "明細", IsBinding = true )]
        SubAcno,
        /// <summary>成本中心</summary>
        [EnumAttribute(FieldName = "成本中心", IsBinding = true)]
        CostCenterID,
        /// <summary>內容</summary>
        [EnumAttribute(FieldName = "內容", IsRequired = true)]
        Describe,
        /// <summary>金額</summary>
    ...
    /// <summary>借貸資料</summary>
    public class BorrowLendTypeFieldProcess : FieldProcess
    {
        public BorrowLendTypeFieldProcess()
        {
            this.ImportFieldKey = SystemImportField.BorrowLendType;
        }

        protected override IFieldBinding GetFieldDictionary()
        {
            string SQL = @"
Select	'1' as ID,'借' as Name
union all
Select	'2' as ID,'貸' as Name
";
    ...
    /// <summary>科目</summary>
    public class AcnoFieldProcess : FieldProcess
    {
        public int CompanyID { get; set; }

        public AcnoFieldProcess()
        {
            this.ImportFieldKey = SystemImportField.Acno;
        }

        protected override IFieldBinding GetFieldDictionary()
        {
            string SQL = @"
Select	Acno as ID,Acno as Name
From	glAccountItem
...
";
    ...
    /// <summary>明細</summary>
    public class SubAcnoFieldProcess : FieldProcess
    {
        public int CompanyID { get; set; }

        public SubAcnoFieldProcess()
        {
            this.ImportFieldKey = SystemImportField.SubAcno;
        }

        protected override IFieldBinding GetFieldDictionary()
        {
            string SQL = @"
Select	SubAcno as ID,SubAcno as Name
From	glAccountItem
where CompanyID=@CompanyID
group by SubAcno
order by SubAcno
