...
namespace TheExcelFileImport
{
    ...
    public class ExcelFileImport
    {
        ...
        public string UserID { get; set; }      //使用者工號
        public string PerfBonusNO { get; set; } //申請單號
        ...
        protected string GetErrorColumnName(SystemImportField aField)
        {
            ...
            return (this.ImportFields.Contains(aField) ? aField : SystemImportField.EmpName).ToString();
        }
        ...
        public TheJsonResult GetTheFileImportResult()
        {
            ...
            //寫入人才庫資料
            if (!DataUpload()) 
                return new TheJsonResult { IsOK = false, ErrorMsg = "上傳資料錯誤", Result = null };
        ...
        protected bool InputValidate(DataTable CheckTable, FieldProcessExecutor FieldValidate)
        {
            ...
            foreach (DataRow aRow in CheckTable.Rows)
            {
                ...
                catch (Exception ex) { aRow.SetColumnError(SystemImportField.EmpName.ToString(), "欄位驗證時錯誤"); }
            }
        ...
        protected void InputValidate2(DataTable CheckTable, FieldProcessExecutor FieldValidate)
        {
            try
            {
                ...
                //刪除已匯入的紀錄
                DeleteBonusImport(connection, transaction);
                //寫入新的匯入紀錄
                InsertBonusImport(connection, transaction, CheckTable);
                //刪除已匯入的紀錄
                //DeleteBonusImport(connection, transaction);
            }
        ...
        protected void InsertBonusImport(IDbConnection connection, IDbTransaction transaction, DataTable aTable)
        {
            try
            {
                //新增寫入
                using (SqlBulkCopy bulkCopy = new SqlBulkCopy((SqlConnection)connection, SqlBulkCopyOptions.KeepIdentity, (SqlTransaction)transaction))
                {
                    ...
                    bulkCopy.DestinationTableName = "dbo.[PerfBonusImport]";
        ...
        protected void DeleteBonusImport(IDbConnection connection, IDbTransaction transaction)
        {
            SqlCommand commandDelete = new SqlCommand("", (SqlConnection)connection, (SqlTransaction)transaction);
            commandDelete.CommandText = @"Delete T From	[dbo].[PerfBonusImport] as T Where	UserID = @UserID";
        ...
        protected bool DataUpload()
        {
            ...
            if (connection.State != ConnectionState.Open) { connection.Open(); }
            IDbTransaction transaction = connection.BeginTransaction();
            string SQL = @"exec dbo.procInsertBonusImportData @UserID,@PerfBonusNO";
            try
            {
                var Parameter = new ArrayList();
                Parameter.Add(new SqlParameter("@UserID", this.UserID));
                Parameter.Add(new SqlParameter("@PerfBonusNO", this.PerfBonusNO));
    ...
    {
        /// <summary>姓名</summary>        
        [EnumAttribute(FieldName = "員工工號", IsRequired = true)]
        EmpID,
        /// <summary>性別</summary>
        [EnumAttribute(FieldName = "員工姓名", IsRequired = true)]
        EmpName,
        /// <summary>電話</summary>
        [EnumAttribute(FieldName = "獎金金額", IsRequired = true)]
        BonusAmt,
