...
namespace _HRM_REC_User_Management
{
    public partial class Component : DataModule
    {
        ...
        public object[] RECUsersQuery(object[] objParam)
        {
            ...
            try
            {
                string SQL = "exec procSearchREC_User N'" + NameC + "','" + Gender + "','" + Age1 + "','" + Age2 + "','" + EduID + "','" + DutyAreas + "','" + CurAddress + "','" + AssignJob + "','" + ProLicenses + "','" + JobCompany + "','" + SalesTeam + "','" + ServiceConsultants + "','" + Status + "','" +
        ...
        //人才失效作業------------------------------------------------------------------------
        public object[] UpdateREC_User(object[] objParam)
        {
            string js = string.Empty;
            //建立資料庫連結
            string sLoginDB = "JBHRIS_DISPATCH";
            ...
            try
            {
                ...
                string SQL = "Update REC_User set RecIsActive=0 from REC_User where UserID='" + UserID + "'";
                this.ExecuteSql(SQL, connection, transaction);
                transaction.Commit();
            }
            catch
        ...
        public void SendAssignMail(string UserID, string NameC, string Email, string AssignMail)
        {
            ...
            if (SendOK)
            {
                //建立資料庫連結
                //IDbConnection connection2 = (IDbConnection)AllocateConnection(GetClientInfo(ClientInfoType.LoginDB).ToString());
                string sLoginDB2 = "JBHRIS_DISPATCH";
                IDbConnection connection2 = (IDbConnection)AllocateConnection(sLoginDB2);
