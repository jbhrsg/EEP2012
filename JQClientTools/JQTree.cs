...
$('#" + this.ClientID + @"').tree({
    onClick: function (node) {
        ...
        if(node.attributes.FORM != null && node.attributes.FORM != '' && node.attributes.FORM != 'null' && node.attributes.FORM.indexOf('http://////') == 0) {
            ...
            if (node.attributes.MODULETYPE == 'W') {
                ...
                addTab(node.text, node.attributes.PACKAGE + '/' + node.attributes.FORM + '.aspx?' + encodeURI(node.attributes.ITEMPARAM));
            }
        }
    ...
    onLoadSuccess: function (node, data) {
        var theTree = $('#" + this.ClientID + @"').tree('collapseAll');        
        $.each(theTree.tree('getRoots'), function (index, node) {
            theTree.tree('expand', node.target);
        });
    },
