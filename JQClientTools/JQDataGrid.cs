...
namespace JQClientTools
{
    ...
    public class JQGridColumn : JQCollectionItem, IJQDataSourceProvider
    {
        ...
        private string DataOptions
        {
            get
            {
                ...
                //jbjob edit by serlina 2014/07/07 預設 dataBox format yyyy/mm/dd
                else if (Editor == JQEditorControl.DateBox)
                {
                    options.Add(string.Format("formatter:{0}", "formatValue"));
                    options.Add(string.Format("format:'{0}'", "yyyy/mm/dd"));
                }
    ...
    public class JQQueryColumn : JQCollectionItem, IJQDataSourceProvider
    {
        ...
        public void Render(HtmlTextWriter writer)
        {
            ...
            // jbjob 2015/2/5 add by serlina    //判斷若為TextArea 可自動換行
            if (Editor != JQEditorControl.TextArea)
                writer.AddAttribute(HtmlTextWriterAttribute.Nowrap, "nowrap");  //禁止文字折返
            writer.RenderBeginTag(HtmlTextWriterTag.Td);
            ...
            // jbjob 2015/2/5 add by serlina    //判斷若為TextArea 
            else if (Editor == JQEditorControl.TextArea)
            {
                ...
                {
                    writer.AddAttribute(HtmlTextWriterAttribute.Style, string.Join(";", string.Format("height:{0}px", textarea.Height.Value)));
                }
