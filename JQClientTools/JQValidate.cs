        ...
        public string Value
        {
            get
            {
                ...
                if (!string.IsNullOrEmpty(CheckMethod))
                {
                    if (RemoteMethod)
                    {
                        values.Add(string.Format("validType:'remote[\\'{0}\\', \\'{1}\\']'", CheckMethod, ValidateMessage));
                    }
                    else
                    {
                        values.Add(string.Format("validType:'client[\\'{0}\\', \\'{1}\\']'", CheckMethod, ValidateMessage));
                    }
                }
                ...
                //jbjob edit by GG 加入年月驗證
                else if (ValidateType == ValidateMode.YYMM)
                {
                    values.Add(string.Format("validType:'jbjobYYMM[]'"));
                }
