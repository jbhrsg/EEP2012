...
namespace JQOfficeTools
{
    ...
    public class Automation
    {
        ...
        public static object[] AnalyzeTag(IOfficePlate op, string sTag) 
        {
            ...
            else if (indexleftb > 0 && indexrightb > indexleftb)
            {
                ...
                for (int i = 0; i < op.DataSource.Count; i++)
                {
                    ...
                    else if ((op.DataSource[i] as DataSourceItem).Caption == (tagname.Split(new char[] { '.' }).Length > 1 ? tagname.Split(new char[] { '.' })[0] : tagname))
                    {
                        if (indexrightb > indexleftb + 1)
                        {
                            string tagparam = sTag.Substring(indexleftb + 1, indexrightb - indexleftb - 1);
                            return new object[] { TagType.DataSource, new object[] { i, tagparam.Trim(), tagname.Split(new char[] { '.' })[1] } };
                        }
                        else
                        {
                            return new object[] { TagType.UnKnown, new object[] { "Recognized as datasource tag, but no params" } };
                ...
                case TagType.DataSource:
                    {
                        ...
                        if (Param.Length > 2)
                            return ProcessDataSourceTag(index, param, Param[2].ToString(), op);
                        else
        ...
        {
            return ProcessDataSourceTag(indexds, param, "", op);
        }
        /// <summary>
        /// Process dataSource tag
        /// </summary>
        /// <param name="indexds">The index of datasource in offeceplate's datasource defination</param>
        /// <param name="param">The parameters of datasource tag</param>
        /// <param name="DetailName">Detail Name for new function,its used in eg:$M.D(c,XXX)</param>
        /// <param name="op">The officeplate whose tags defination is used</param>
        /// <returns>Object consist of plateexception and value</returns>
        public static object[] ProcessDataSourceTag(int indexds, string param, string DetailName,IOfficePlate op)
        {
            ...
            //int rowindex = -1;
            DataView view = (DataView)((DataSourceItem)op.DataSource[indexds]).GetTable()[DetailName != "" ? DetailName : ((DataSourceItem)op.DataSource[indexds]).DataMember];
            DataRow row = null;
            if(string.Compare(strparam[0].Trim(), "c",true) == 0)
            {
                if (((DataSourceItem)op.DataSource[indexds]).DataSource is DataSet)
                {
                    if (DetailName != "")
                    {
                        var dataSet = (op.DataSource[indexds]).DataSource as DataSet;
                        if (dataSet.Tables[DetailName] != null && dataSet.Tables[DetailName].Rows.Count > 0)
                        {
                            row = dataSet.Tables[DetailName].Rows[0];
                        }
                        else
                        {
                            return new object[]{PlateException.InvalidParameter
                    ,string.Format("The first parameter of DataSource:'{0}' is invalid,'{1}' is invalid",((DataSourceItem)op.DataSource[indexds]).Caption,DetailName)};
                        }
                    }
                    else if ((((DataSourceItem)op.DataSource[indexds]).DataSource as DataSet).Tables[0].Rows.Count > 0)
                    {
                        row = (((DataSourceItem)op.DataSource[indexds]).DataSource as DataSet).Tables[0].Rows[0];
                    }
                    else
                    {
                        return new object[]{PlateException.InvalidParameter
                    ,string.Format("The first parameter of DataSource:'{0}' is invalid,'{1}' has not datas",((DataSourceItem)op.DataSource[indexds]).Caption,strparam[0])};
            ...
            {
                int indeximagecolumn = -1;
                for (int i = 0; i < ((DataSourceItem)op.DataSource[indexds]).ImageColumns.Count; i++)
                {
                    if (string.Compare((((DataSourceItem)op.DataSource[indexds]).ImageColumns[i] as DataSourceImageColumnItem).ColumnName, column, true) == 0)
                    {
                        indeximagecolumn = i;
                        break;
                    }
    ...
    public class Expression
    {
                ...
                if (!strop.Contains(member))
                {
                    ...
                    else if (member.Split('{', '}').Length == 3)
                    {
                        ...
                        if(op is OfficePlate)
                        {
                            ...
                            obj = new object[1] { PlateException.InvokeMethodError };
                        }
                        else
                        {
                    ...
                    if (objmember is string || objmember is DateTime || objmember is bool || objmember.Equals(DBNull.Value))
                    {
                        isNumExp = false;
                    }
                    list[i] = objmember;
        ...
        public static object GetSysParameters(string sysparam)
        {
            switch (sysparam.ToLower())
            {
                ...
                case "_userid": return EFClientTools.ClientUtility.ClientInfo.UserID;
                case "_username": return EFClientTools.ClientUtility.ClientInfo.UserName;
                case "_sitecode": return EFClientTools.ClientUtility.ClientInfo.Site;
                case "_groupid": return EFClientTools.ClientUtility.ClientInfo.CurrentGroup;
                case "_database": return EFClientTools.ClientUtility.ClientInfo.Database;
                case "_ipaddress": return EFClientTools.ClientUtility.ClientInfo.IPAddress;
                case "_solution": return EFClientTools.ClientUtility.ClientInfo.Solution;
                case "_language": return EFClientTools.ClientUtility.ClientInfo.Locale;
                default: return null;
            }        
        }
