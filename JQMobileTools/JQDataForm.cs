...
namespace JQMobileTools
{
    ...
    public class JQDataForm : WebControl, IJQDataSourceProvider, IColumnCaptions, IDetailObject, IThemeObject
    {
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            if (!this.DesignMode)
            {
                    ...
                    || this.Page.Request.QueryString["NAVMODE"].ToString() == "Inquery")))
                {
                    ...
                    if (IsControlVisible("Plus") && (string.IsNullOrEmpty(this.Page.Request.QueryString["PLUSROLES"]) || this.Page.Request.QueryString["PLUSROLES"].ToLower() == "null"))
                    {
                        writer.RenderBeginTag(HtmlTextWriterTag.Td);
                    ...
                    if (!IsNotifyOFF && IsControlVisible("Notify") && IsClose(this.Page.Request.QueryString["LISTID"]))
                    {
                        writer.RenderBeginTag(HtmlTextWriterTag.Td);
        ...
        private void InitStates()
        {
            ...
            string flowFileName = this.Page.Request.QueryString["FLOWFILENAME"];
            string status = this.Page.Request.QueryString["STATUS"];
            string flNavigatorMode = this.Page.Request.QueryString["FLNAVMODE"];
            if (String.IsNullOrEmpty(flNavigatorMode))
            {
                flNavigatorMode = this.Page.Request.QueryString["FLNAVIGATOR_MODE"];
            }
            string navigatorMode = this.Page.Request.QueryString["NAVIGATOR_MODE"];
            if (String.IsNullOrEmpty(navigatorMode))
            {
                navigatorMode = this.Page.Request.QueryString["NAVMODE"];
            }
        ...
        public bool IsControlVisible(string ctrlName)
        {
            ...
            else if (ctrlName == "Plus")
            {
                ...
                {
                    case "Approve":
                        if (this.Page.Request.QueryString["PLUSAPPROVE"] != null && this.Page.Request.QueryString["PLUSAPPROVE"].ToString() == "1")
                            visible = true;
                        else
                            visible = false;
                        break;
                    ...
                    case "RSubmit":
                        ...
                        break;
                    case "Plus":
                        if (this.Page.Request.QueryString["STATUS"] != null && this.Page.Request.QueryString["STATUS"].ToString() == "AA")
                            visible = true;
                        else
                            visible = false;
                        break;
