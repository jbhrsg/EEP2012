    ...
    public partial class TJQMobileFormGenerator
    {
        ...
        private void InsertControl(WebDevPage.IHTMLElement pageElement, Type controltype, string id)
        {
            if (pageElement != null)
            {
                ...
                {
                    builder.AppendLine(string.Format("<{0}:{1} ID=\"{2}\" runat=\"server\">", "JQMobileTools", controltype.Name, id));
        ...
        private string GetControlXml(System.Web.UI.Control control, PropertyInfo prop, string field, string format)
        {
            ...
            if (control != null)
            {
                ...
                {
                    builder.Append(string.Format("<{0}:{1} ID=\"{2}\" runat=\"server\" ", "JQMobileTools", controltype.Name, control.ID));
                }
                if (prop != null)
                {
                    builder.Append(string.Format("{0}='<%# Bind(\"{1}\"", prop.Name, field, format));
                ...
                for (int i = 0; i < infos.Length; i++)
                {
                    if (prop == null || (prop != null && prop.Name != infos[i].Name))
                    {
                        if (!IsVisibilityHidden(infos[i]))
                        {
                            ...
                            else if (infos[i].PropertyType.BaseType == typeof(JQCollection<JQCollectionItem>))
                            {
                                string collectionxml = GetCollectionXml(infos[i], (IJQProperty)infos[i].GetValue(control, null));
                                if (collectionxml.Length > 0)
                                {
