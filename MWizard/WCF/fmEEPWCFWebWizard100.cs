...
namespace MWizard
{
    ...
    public partial class TWCFWebClientGenerator
    {
#if VS90
        /// <summary>
        /// "Infolight"字串
        ...
        private void InsertControl(WebDevPage.IHTMLElement pageElement, System.Web.UI.Control control)
        {
            if (pageElement != null)
            {
                ...
                {
                    string html = pageElement.innerHTML;
                    int index = IndexOfEndTag(html, "div");
                    if (index != -1)
                    {
                        pageElement.innerHTML = html.Insert(index, controlxml);
                    }
        ...
        private void SetCollectionValue(WebDevPage.IHTMLElement controlElement, PropertyInfo prop, System.Collections.IList collection)
        {
            if (controlElement != null)
            {
                string collectionxml = GetCollectionXml(prop, collection);
        ...
        private string GetControlXml(System.Web.UI.Control control, PropertyInfo prop, string field, string format)
        {
            ...
            if (control != null)
            {
                ...
                else if (controltype.Namespace == "ExtTools")
                {
                    builder.Append(string.Format("<{0}:{1} ID=\"{2}\" runat=\"server\" ", "ExtTools", controltype.Name, control.ID));
                }
                else if (controltype.Namespace == "EFClientTools.Web")
                {
                    builder.Append(string.Format("<{0}:{1} ID=\"{2}\" runat=\"server\" ", "EFClientTools", controltype.Name, control.ID));
                }
                else
                {
                    builder.Append(string.Format("<{0}:{1} ID=\"{2}\" runat=\"server\" ", "Asp", controltype.Name, control.ID));
                ...
                for (int i = 0; i < infos.Length; i++)
                {
                    if (prop == null || (prop != null && prop.Name != infos[i].Name))
                    {
                        if (!IsVisibilityHidden(infos[i]))
                        {
                            ...
                            {
                                string collectionxml = GetCollectionXml(infos[i], (System.Collections.IList)infos[i].GetValue(control, null));
                                if (collectionxml.Length > 0)
                                {
                                    builderInnerHtml.AppendLine(collectionxml);
                                }
                ...
                {
                    builder.AppendLine(string.Format("</{0}:{1}>", "ExtTools", controltype.Name));
                }
                else if (controltype.Namespace == "EFClientTools.Web")
                {
                    builder.AppendLine(string.Format("</{0}:{1}>", "EFClientTools", controltype.Name));
                }
                else
                {
                    builder.AppendLine(string.Format("</{0}:{1}>", "Asp", controltype.Name));
        ...
        private string GetControlXml(TBlockFieldItem item, string tableName, ref string id)
        {
            ...
            if (string.Compare(item.ControlType, "combobox", true) == 0)
            {
                ...
                (control as WebDropDownList).DataSourceID = GenEFDataSource(item, tableName, "ComboBox", string.Empty);
...
#warning GenWebDataSource未完成FSYS_REFVAL部分
                ...
                {
                    (control as WebRefVal).DataSourceID = GenEFDataSource(item, tableName, "RefVal", string.Empty);
        ...
        /// <param name="collection">Collection的value</param>
        /// <returns>xml</returns>
        private string GetCollectionXml(PropertyInfo prop, System.Collections.IList collection)
        {
            ...
            if (prop != null && collection != null && prop.PropertyType == collection.GetType())
            {
                if (collection.Count > 0)
                {
                    ...
                    for (int i = 0; i < collection.Count; i++)
                    {
                        ...
                        if (collection[i] is ExtTools.ExtFormField || collection[i] is ExtTools.ExtGridColumn
                            || collection[i] is ExtTools.ExtQueryField || collection[i] is ExtTools.ExtSimpleColumn)
                        {
                            builder.Append(string.Format("\t\t<{0}:{1} ", "ExtTools", collection[i].GetType().Name));
                            infos = collection[i].GetType().GetProperties();
                        }
                        else
                        {
                            builder.Append(string.Format("\t\t<{0}:{1} ", INFOLIGHTMARK, collection[i].GetType().Name));
                            infos = collection[i].GetType().GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                        }
