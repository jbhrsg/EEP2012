    ...
    public partial class TJQueryWebFormGenerator
    {
        ...
        private void InsertControl(WebDevPage.IHTMLElement pageElement, Type controltype, string id)
        {
            if (pageElement != null)
            {
                ...
                {
                    builder.AppendLine(string.Format("<{0}:{1} ID=\"{2}\" runat=\"server\">", "JQTools", controltype.Name, id));
        ...
        private string GetControlXml(System.Web.UI.Control control, PropertyInfo prop, string field, string format)
        {
            ...
            if (control != null)
            {
                ...
                {
                    builder.Append(string.Format("<{0}:{1} ID=\"{2}\" runat=\"server\" ", "JQTools", controltype.Name, control.ID));
                }
                if (prop != null)
                {
                    builder.Append(string.Format("{0}='<%# Bind(\"{1}\"", prop.Name, field, format));
